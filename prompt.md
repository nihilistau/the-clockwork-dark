...
It is more than one game: a reusable **engine** ([[clockwork-engine]]) you can
retarget to a new story ([[building-on-the-engine]]).

Before you code:
1. Read `knowledge/index.md` and follow the links relevant to your task.
2. Skim [[okfs-spec]] (the format), [[clockwork-engine]] (the engine), and
   [[clockwork-architecture]] (the project).
3. Honor the project rules (CLAUDE.md / AGENTS.md): the **engine resolves all
   ...
   `get_config()`); reuse before rewriting; keep `pytest` green; capture durable
   knowledge as a new OKFS concept, keep the bundle valid (`OKFSBundle.validate()`),
   refresh the index (`python scripts/build_okfs_index.py`), and log notable
